"""This module defines the Student and StudentManager classes
for managing student data.The Student class represents a
student with their name, gender,and scores in various subjects,
while the StudentManager class provides methods to add, update,
delete, view, search, and filter students. The StudentManager class
also handles saving and loading student data to and from a
JSON file, ensuring that the data persists across sessions.
The application is designed to be used in a terminal environment,
with a menu-driven interface for user interaction."""
...
@dataclass
class Student:
    """This class represents a student with their name, gender,
    and scores invarious subjects. The __str__ method is overridden
    to provide a readable string representation of the student
    object when printed. dataclass is used to automatically generate
    special methods like __init__ and __repr__,making it easier
    to create and manage student objects."""
    ...
    def __str__(self):
        return (
            ...
            f"English Score: {self.english_score} | "
            f"Math Score: {self.math_score} | "
            f"Science Score: {self.science_score} | "
            f"Art Score: {self.art_score} | "
            f"Total score: {self.total_score()}"
        )

    # Method to calculate total score of the student by
    # summing up the scores in all subjects.

    def total_score(self) -> int:
        """Calculates the total score of the student by
        summing up the scores in all subjects."""
        return (
            self.english_score
            + self.math_score
...
class StudentManager:
    ...
    def add_student(self, student: Student) -> int:
        """Adds a new student to the manager and
        returns the assigned student ID."""
        student_id = self.next_id
        # Assign the generated ID to the student object
    ...
    # It checks if the name is not empty and contains
    # at least two parts (first name and last name).

    def _validate_name(self, name: str, allow_empty: bool = False) -> bool:
        """Validates that the name is a valid full name (first and last name)
        or empty if allow_empty is True."""
        if not name.strip():
            return allow_empty
        parts = name.split()
        if len(parts) < 2:
            return False
        return all(re.fullmatch(r"[A-Za-z][A-Za-z'-]*", part)
                   for part in parts)
    ...
    def _validate_gender(self, gender: str, allow_empty: bool = False) -> bool:
        """Validates that the gender is either 'Male' or 'Female'
        or empty if allow_empty is True."""
        if not gender.strip():
            return allow_empty
    ...
    # which is an integer between 0 and 100,
    # or it's empty when allow_empty is set to True.

    def _validate_score(self, score: str, allow_empty: bool = False) -> bool:
        """Validates that the score is an integer between 0 and 100
        or empty if allow_empty is True."""
        if not score.strip():
            return allow_empty
    ...
    # Method to read input with given prompt until a valid input is provided,
    # it uses the provided validation function
    # to check the validity of the input
    # and displays the given error message if the input is invalid.

    def _read_validated_input(
        self,
        prompt: str,
        validation_func,
        error_message: str,
    ...
    def read_student_details(self, is_update=False) -> dict:
        """Reads student details from user input
        and returns a dictionary of the details."""
    ...
    def from_dict(self, data: dict) -> Student:
        ...
        return Student(
            # ID will be assigned when the student is added to the manager
    ...
    def add_student_from_dict(self, data: dict) -> int:
        """Adds a student to the manager from a dictionary of student details
        and returns the assigned student ID."""
        student = self.from_dict(data)
        return self.add_student(student)

    # Read student details from the user and create a new student object,
    # then add it to the manager.

    def create_student(self):
        """Creates a new student by reading details from user input
        and adds it to the manager."""
        print("Enter details for the new student:\n")
        student = self.from_dict(self.read_student_details())
    ...
    def update_student(self, student_id: int):
        ...
        student.name = (
            updated_data["name"] if updated_data["name"] else student.name
        )
        student.gender = (
            updated_data["gender"]
        ...
        student.english_score = (
            int(updated_data["english_score"])
            if updated_data["english_score"]
            else student.english_score
        )
        student.math_score = (
            int(updated_data["math_score"])
            if updated_data["math_score"]
            else student.math_score
        )
        student.science_score = (
            int(updated_data["science_score"])
            if updated_data["science_score"]
            else student.science_score
        )
        student.art_score = (
            int(updated_data["art_score"])
            if updated_data["art_score"]
            else student.art_score
        )
    ...
    def delete_student(self, student_id: int):
        ...
        confirmation = input(
            f"Do you want to delete the student with"
            f" ID {student_id}?(yes/no): "
        )
    ...
    def load(self, file_path: str = None):
        ...
        if not Path(self.file_path).exists():
            print(f"No existing data file found at {self.file_path}")
            return
        with open(self.file_path, "r", encoding="utf-8") as f:
            ...
            for student_data in students_data:
                ...
                # confirm next_id is always greater than
                # the highest existing ID
                self.next_id = max(self.next_id, student.id + 1)

    # Method to filter students by Marks ,whose got DISINCTION,
    # FIRST CLASS, SECOND CLASS, THIRD CLASS and FAIL
