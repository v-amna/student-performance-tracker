"""This module defines the Student and StudentManager classes for managing student data.
The Student class represents a student with their name, gender, 
and scores in various subjects, while the StudentManager class provides methods to
add, update, delete, view, search, and filter students. The StudentManager class also
handles saving and loading student data to and from a JSON file, ensuring that the
data persists across sessions. The application is designed to be used in a terminal
environment, with a menu-driven interface for user interaction."""
from dataclasses import dataclass
import json
from pathlib import Path
@dataclass
class Student:
    """This class represents a student with their name, gender, and scores in
        various subjects. The __str__ method is overridden to provide a readable
        string representation of the student object when printed. dataclass is used
        to automatically generate special methods like __init__ and __repr__,
        making it easier to create and manage student objects."""
    ...
    def __str__(self):
        return f"Id: {self.id} | Name: {self.name} | Gender: {self.gender} | English Score: {self.
        english_score} | Math Score: {self
        .math_score} | Science Score: {self.science_score} | Art Score: {self.
        art_score} | Total score: {self.total_score()}"
...
class StudentManager:
    ...
    def _read_validated_input(self, prompt: str, validation_func, error_message: str,
    ...
    def update_student(self, student_id: int):
        ...
        # Save the updated students data to the JSON file after updating a student
        self._save()
    ...
    def delete_student(self, student_id: int):
        ...
        if confirmation.lower() == "yes":
            if student_id in self.students:
                ...
                # Save the updated students data to the JSON file after deleting a student
                self._save()
            else:
                print(f"Student with ID {student_id} does not exist.")
        else:
    ...
    def load(self,file_path: str = None):
        ...
        with open(self.file_path, 'r', encoding='utf-8') as f:
            ...
            for student_data in students_data:
                ...
                # confirm next_id is always greater than the highest existing ID
